...
public class Terrain : MonoBehaviour
{
    ...
    public virtual void Generate(int size)
    {
        ...
        for (int i = 1; i <= darkenAmount; i++)
        {
            var leftTile = SpawnTile(-moveLimit - i);
            var rightTile = SpawnTile(moveLimit + i);

            DarkenObject(leftTile);
            DarkenObject(rightTile);
        }

        var leftBoundaryTile = SpawnTile(-moveLimit - darkenAmount - 1);
        var rightBoundaryTile = SpawnTile(moveLimit + darkenAmount + 1);
